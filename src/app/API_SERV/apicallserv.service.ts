...
export class ApicallservService {
  ...
  private port = environment.portaddres
  ...
  fn_postMe(path: string = "", param: object, port?: string): Observable<any> {
    let options = {
      headers: this.setHeaders()
    }
    return this.http.post(this.myurl(this.host, port, path), param, options)
  }
  fn_Delete(path: string = "", param: object, port ? : string): Observable < any > {
    let options = {
      headers: this.setHeaders(),
      body:param
    };
  ...
  fn_ImageUpload(path: string = "", param, port ? : string): Observable < any > {
